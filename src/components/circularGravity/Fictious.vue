...
<template>
    <div class="container h100 p-5">
        ...
        <p>
            <div v-show="page===0">
                Remember the centrifugal force from earlier? Well, I said that it doesn’t exist, but it does in a mathematical sense. 
                For an observer at rest, it does not exist, but the observer actually going in a circle experiences it. If you've ever been
                ...
                (Newton’s first law!). It is also oppositely directed to the actual acceleration of the object. This might be 
                a bit confusing, but think of the times when you've felt a force pushing you back into the car seat when someone hits the
                gas pedal (even though the car is accelerating forward). Again, I want to emphasize that this force technically
                does not actually exist and is sometimes compared to an "invisible hand". However, when you consider your reference frame this force is very
                ...
                <div v-show="level>0">
                    ...
                    <br><br>
                    The elevator accelerates <i>upward</i>, so any object inside it experiences a <i>downward</i> fictitious 
                    ...
                    (if you don’t know this already from riding elevators). You can even use a scale to prove it to yourself.
                    ...
                    <br><br>
                    We know that there are only two (real) forces acting on the system, being the gravitational and normal forces. 
                    Gravity points downwards and the normal force upward, We are going to consider the case where the elevator 
                ...
                <div v-show="level==0">
                    ...
                    <br><br>
                    This is one of those cases where actually having been in an elevator helps a lot. If you've ever been in
                    an elevator that is accelerating upwards, whether that be speeding up on the way up or slowing down on the way down, you feel
                    yourself become heavier. Your mass isn't actually increasing since that's an intrinsic property and doesn't change so easily. You're
                    ...
                    We can work in the accelerated reference frame of the object moving in a circle, which gives rise to a fictitious centrifugal force 
                    (it’s back!) that we can use to simplify calculations. If we treat problems in this way, they become conceptually simpler force balance problems
                    akin to those covered in the last unit. Recall our banked curve. We can add a centrifugal force to our force-vector diagram and treat it as a
                    classic force-balance problem. This might seem simpler for some of you, but you might also find the method unnecessary in that case. Well, we are next
            ...
            <div v-show="page===1">
                ...
                <div v-show="level>0">
                    ...
                    <br><br>
                    Now, the first part of this question is a bit simpler to answer. You have a single possible velocity for
                    a given acceleration. Now, it might be tricky to figure out what the centripetal force is at the top, but
                    remember that hills are vertical. The normal force varies since the net force has to equal the required centripetal
                    force for circular motion at a certain velocity, and it opposes gravity. At the critical point, there is no more normal force and the entirety of the
                    gravitational force contributes to the centripetal force. Therefore, at the top, the maximum possible centripetal
                    ...
                    $v = \sqrt{gR}$
                    <br><br>
                    Technically, this is a maximum value since the normal force can vary and allow for other velocities. If you
                    prefer, it is more accurate to write:
                    <br><br>
                    $v \leq \sqrt{gR}$
                    <br><br>
                    Now, this part did not require any invocation of ficticious forces, but the second part of the problem is easier
                    if you invoke them. Think about how the ficticious centrifugal force would be directed. In the critical case, it is
                ...
                <div v-show="level==0">
                    ...
                    <br><br>
                    At the top of the hill, the radius of the hill is vertical, so we are only concerned with forces in the vertical direction.
                    The only forces that can be in this direction are the gravitational and normal forces, and they are oppositely directed. In
                    this scenario, $F_n < mg$ because the net force has to be downward to provide sufficient centripetal force for circular
                    motion to occur. This is yet another case where the normal force is not equal to the weight of the object! For speeds low
                    enough, the car will remain in contact with the ground, but there is a critical point where the normal force is zero and the
                    entirety of the gravitational force is required to balance out the required centripetal force (or centrifugal force). This
                    gives us the equation (it's one of the only ones you'll see here, I promise!):
                    <br><br>
                    $m\dfrac{v^2}{R} = mg$
                    <br><br>
                    Now, you can simplify this rather easily, but if you don't feel comfortable doing it, don't worry!
                    You just need to know that the critical velocity where the normal force is zero is:
                    <br><br>
                    $v = \sqrt{gR}$
                    <br><br>
                    This is the maximum velocity at which circular motion is still possible. What happens if you go faster than this? Well,
            ...
            <div v-show="level>0">
                ...
                <br><br>
                This problem is not trivial. It takes significant thought to figure out why this would even be possible
                in the first place, since there is no friction. Most people see $m_3$ and conclude that it is impossible
                for it to stay in place without any frictional forces. However, with ficticious forces things begin to
                make sense.
                <br><br>
                The best approach to the problem is actually to start with the problematic block, $m_3$. It can easily accelerate
                horizontally from a normal force applied on it by the big cart $m_1$, but it msut remain still vertically. We can actually
                ignore this horizontal acceleration because the system moves as a whole, and it doesn't offer us any particularly useful
                information. Since the block must remain at rest vertically, the tension force $T$ must obey the relation:
                <br><br>
                $T = m_3g$
                <br><br>
                Now, you might be really confused. For a normal Atwood's machine in this setup, the tension force is not this! However, this 
                is the only way the third block can stay at rest. This leads us to the conclusion that this tension force also acts on
                the second block. Now is where we invoke the ficticious force. Since the cart accelerates to the right due to a force
                being exerted on it, we can assume it must have some acceleration $a$. This leads to a ficticious force to the left of 
                magnitude $m_2a$ "applied" onto the second block, which must be balanced by the tension force. Thus, we have:
                <br><br>
                $T = m_2a$
                <br><br>
                ...
                $a = \dfrac{m_3}{m_2}g$
                <br><br>
                Now, we consider the force $F$. Since we have said that the system moves as a whole without any relative motion,
                we can treat it as one body and only consider the external forces on it (which is just $F$ in this scenario):
                <br><br>
                $F = (m_1+m_2+m_3)a$
                <br><br>
                ...
                <br><br>
                That was difficult, right? But it's also an interesting problem that raises some interesting implications. Accelerated
                reference frames can really mess with your usual intuitions, and postulates you held to be always true might not be
                true when working with one! 
                <br><br>
                There is another interesting question I'd like to pose about this scenario. If all surfaces are frictionless, then how
            ...
            <div v-show="level==0">
                ...
                <br><br>
                We want to see if it's possible to push the cart such that none of the block slip or move relative to each
                other. In other words, we want the cart and blocks to remain in place as we push it. Now, normally this
                would seem impossible without friction, since $m_2$ cannot stop $m_3$ from sliding down because it is placed
                horizontally. However, the blocks are accelerating.
                <br><br>
                Since there is a net external force represented by $F$ on the system, it will tend to accelerate to the right. This
                means there will be a ficticious force to the left on each block. The only way block 3 can remain at rest is if the
                tension force is equal to its weight, so we can assume that. Next, we know that this tension force is the only real
                force acting in the horizontal direction on the second block. It is directed to the right, so we know that it has to
                balance the ficticious force on the second block. This enables us to do force balance and makes the scenario physically possible.
                <br><br>
                The result we get from calculations is that there is only one possible acceleration where the system can remain at rest
                relative to itself, which means by Newton's second law there is only one possible force $F$ that will enable this kind
                of motion. An interesting result, to be sure.
                <br><br>
                We know that all three blocks accelerate together, so we can treat them as one object. But for them to move together, they
                have to interact with each other through some force. However, it is not obvious how block 2 would interact with the cart. This
