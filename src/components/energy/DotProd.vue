    ...
    <div class="container h100 p-5">
        ...
        <p>
            <div v-show="page===0">
                ...
                <br><br>
                <span v-show="level>0">
                    The dot product somewhat describes the projection of one vector onto another. It's denoted with a big old
                    dot like this: $\vec{a} \cdot \vec{b}$. More accurately, it technically better describes
                    how parallel or orthogonal (perpendicular) two vectors are to one another, with a value of one meaning the vectors are entirely
                    parallel, a value of zero meaning the vectors are orthogonal, and a value of negative one meaning the vectors are 
                    antiparallel. Notice how these values are all scalars! Indeed, the dot product takes two vectors and outputs a scalar
                    ...
                    <br><br>
                    Unlike adding or subtracting vectors, with the dot product there is no real way to perform it graphically. (This makes
                    sense, since you're outputting a scalar and we don't typically represent scalars on graphs.) The first way to do so
                    ...
                    one and negative one when the vectors are parallel and antiparallel, repsectively. This matches up with what I talked about before. Additionally, if the
                    angle $\theta$ is a right angle, then the cosine is zero and the dot product evaluates to zero, which matches the orthogonality
                    condition I touched on briefly. 
                    <br><br>
                    Any values between these will evaluate to something (with an absolute value) less than one but greater than zero,
                    which covers all of the cases where the vectors aren't perfectly parallel or perpendicular. Vectors that are more parallel will
                    have a smaller angle $\theta$ between them, which evaluates to a higher value of cosine and vice versa. So, with just a bit of analyzing a basic trig function (which you should know
                    ...
                    product is equal to the component of $\vec{a}$ on $\vec{b}$ multiplied by the magnitude of $\vec{b}$. You'll
                    see how important this is later on.
                    <br><br>
                    Just presenting one form of this operator is a bit limiting, however. Therefore, I'll be showing the other
                ...
                <span v-show="level==0">
                    ...
                    how parallel or perpendicular the two vectors are, with a greater value meaning more parallel vectors. A negative
                    value means the vectors are pointing in opposite directions (antiparallel). Let's use a diagram to illustrate the
                    ...
                    <br><br>
                    Now, this part requires a bit of trig knowledge. Remember our vectors lesson, and the idea of components.
                    ...
                    <br><br>
                    We can analyze this function by analyzing the cosine function, but this might be a bit too mathematical. Instead,
                    we're just going to recognize that the cosine can range from negative one to one, allowing for a range of values for the dot
                    product. Moreover, the dot product is really just equal to the product of the component of $\vec{a}$ on $\vec{b}$ and the
                    magnitude of $\vec{b}$. This will come in handy for physics problems.
                    <br><br>
                    This is really enough knowledge of the dot product for physics, but for the sake of having a thorough lesson let's talk
            ...
            <div v-show="page===1">
            The other form of the dot product requires the use of vector components. While in physics you're more commonly 
            given the angle between two vectors of interest, in math you're mostly given vectors in component form. Additionally,
            it's not unheard of to give vector components in physics, so it'll be good to know this second form of the dot product.
            <br><br>
            ...
            <br><br>
            <span v-show="level>0">
                In fact, while the first method of computing the dot product would technically work here, it would only be convenient for vectors that were 
                ...
                talking about here), using vectors can allow for mathematical shortcuts and the "extra" dimensions are often used in
            ...
            <br><br>
            <div class="problem">
                Find the dot product of two vectors $\vec{o} = \langle 3, 12, -1 \rangle $ and $\vec{p} = \langle -4, 0.5, 2 \rangle$.
                <br><br>
            ...
            <br><br>
            This is definitely the easier of the two vector products, involving pretty straightforward calculations. This lesson
            was pretty short as a result, but you still need to make sure that you're clear on how the dot product works.
            Why? The next lesson will cover our first physics concept of this unit, which coincidentally relies on the dot product! I wonder
            why that is...
